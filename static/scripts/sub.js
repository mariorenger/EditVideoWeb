var id_video_add_sub;

var modal = document.getElementById("myModal");
var btn = document.getElementById("myBtn");
var span = document.getElementsByClassName("close-sub")[0];

btn.onclick = function() {
  modal.style.display = "block";
  const targetNode = window.references[window.rightClickCtx.target.id]
  id_video_add_sub=window.rightClickCtx.target.id
  console.log(targetNode.data.metadata.path)
  document.getElementById('video').setAttribute('src', targetNode.data.metadata.path);
}
...
var _cues = document.getElementById('cues');
var _vttFilename = '';
...
document.getElementById('playPause').on('click', function() {
    togglePlayPause();
});
initVideo();
...
document.getElementById('addCue').on('click', function() {
    var row;
...
document.getElementById('exportVTT').on('click', function() {
    let video = document.getElementById(id_video_add_sub);
    let track;
    ...
    // delete all list old cue and add all list new cue
    oldCueList = track.cues;
    for(let oldCue of oldCueList){
        track.removeCue(oldCue);
    }

    track.mode = "showing";
    document.getElementById('sortCues').click();
...
document.getElementById('previewButton').on('click', function() {
...
document.getElementById('sortCues').on('click', function() {
...
document.getElementById('importVideo').on('click', () => {
    document.getElementById('importVideoFile').click();
});
document.getElementById('importVideoFile').on('change', function(event) {
    var file, url;
...
document.getElementById('importVTT').on('click', function() {
    document.getElementById('importVTTFile').click();
});
document.getElementById('importVTTFile').on('change', function(event) {
...
function onDrop(event) {
    ...
    if (file.type.indexOf('text/') === 0) {
        input = document.getElementById('importVTTFile');
    } else if (file.type.indexOf('video/') === 0) {
        input = document.getElementById('importVideoFile');
    } else {
        return;
    }
    input.files = files;
